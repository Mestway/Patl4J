...
\begin{theorem}[Def-use Preservation\footnote{Due to space limit, the proofs of all theorems or properties are omitted, please refer to \url{https://github.com/Mestway/Patl4J/blob/master/patl-full.pdf} for full version with proofs.}] Given a method $M$ and a checked \PATL program $\Pi$, suppose the statement-level transformation is $\Pi\triangleright \bar{s}_M=\bar{s}'_{M}$, then:
  \begin{enumerate}
