...
public class WorkspaceController {
	...
	public Document getWorkspaceData(){ 
		Element rootElement = DocumentHelper.createElement("husacct"); 
		rootElement.addAttribute("version", "3.4.1");		 
		...
		try {
			...
			//if(document.hasRootElement()){
				...
				for(ISaveable service : savableServices){
					String serviceName = service.getClass().getName();
					List<Element> elementQuery = rootElement.elements(serviceName); 
					for(Element serviceDataContainer : elementQuery){ 
						Element serviceData = serviceDataContainer.elements().get(0);  
