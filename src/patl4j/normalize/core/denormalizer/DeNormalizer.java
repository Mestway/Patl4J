package patl4j.normalize.core.denormalizer;
...
public class DeNormalizer extends ASTVisitor {
	...
	void replaceNode(Expression exp, Expression newExp) {
		...
		//	System.out.println("@Name -> " + "Replace var " + name + " with exp " + newExp);
			System.out.println(newExp.getClass().getName());
			FieldAccess tmp = p.getAST().newFieldAccess();
			SimpleName name = ((QualifiedName) p).getName();
			tmp.setName(name);
			tmp.setExpression(newExp);
			//((QualifiedName) p).setName((SimpleName) newExp);
		} */
	}
